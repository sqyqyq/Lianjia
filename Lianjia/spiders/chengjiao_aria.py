...
class Chengjiao_ariaSpider(scrapy.Spider):
    name = 'chengjiao_aria'
                ...
                for k in range(len(links_city)):
                    ...
                    request = scrapy.Request(link_chengjiao,
                                             ...
                                             cb_kwargs={'province': province, 'city': city,
                                                        'link_chengjiao': link_chengjiao, 'bp_aria': bp_aria,
                                                        'ep_aria': ep_aria, 'stl_large': False, 'host': host})

                    yield request
            # else:
            #     break

    def parse_pagelinks(self, response, province, city, link_chengjiao, bp_aria, ep_aria, stl_large, host):
        ...
        if total is None:
            ...
            if '人机认证' in n_title:
                ...
                request = scrapy.Request(response.url,
                                         ...
                                         cb_kwargs={'province': province, 'city': city,
                                                    'link_chengjiao': link_chengjiao, 'bp_aria': bp_aria,
                                                    'ep_aria': ep_aria, 'stl_large': stl_large, 'host': host})
                yield request
            else:
                request = scrapy.Request(response.url,
                                         # meta={'dont_redirect': True, 'handle_httpstatus_list': [302]},
            ...
            if total_num != 0:
                if total_num <= 3000 or stl_large:
                    ...
                    with open('stat1.csv', 'a+', encoding='utf-8-sig', newline="") as csvfile:
                        writer = csv.writer(csvfile)
                    ...
                    for x in range(total_page):
                        ...
                        request = scrapy.Request(link_page,
                                                 ...
                                                 cb_kwargs={'province': province, 'city': city,
                                                            'link_chengjiao': link_page, 'host': host, 'check': check})
                        yield request
                else:
                    # 如果大于3000 则开始定义filter 使其小于3000
                    # 这里使用 售价和用途来划分 不容易丢失或者重复数据
                    if (ep_aria - bp_aria) > 1:
                        mid_aria = int((ep_aria + bp_aria) / 2)
                        left_link_page = link_chengjiao + "ba" + str(bp_aria) + "ea" + str(mid_aria) + "/"
                        right_link_page = link_chengjiao + "ba" + str(mid_aria) + "ea" + str(ep_aria) + "/"
                        ...
                        request_left = scrapy.Request(left_link_page,
                                                      ...
                                                      cb_kwargs={'province': province, 'city': city,
                                                                 ...
                                                                 'bp_aria': bp_aria,
                                                                 'ep_aria': mid_aria, 'stl_large': stl_large,
                                                                 'host': host})
                        request_right = scrapy.Request(right_link_page,
                                                       ...
                                                       cb_kwargs={'province': province, 'city': city,
                                                                  'link_chengjiao': link_chengjiao,
                                                                  'bp_aria': mid_aria,
                                                                  'ep_aria': ep_aria, 'stl_large': stl_large,
                                                                  'host': host})
                        yield request_left
                        yield request_right
                    else:
                        ...
                        for u in usage:
                            new_link_page = link_chengjiao + u + "ba" + str(bp_aria) + "ea" + str(ep_aria) + "/"
                            yield scrapy.Request(new_link_page,
                                                 ...
                                                 cb_kwargs={'province': province, 'city': city,
                                                            'link_chengjiao': link_chengjiao, 'bp_aria': bp_aria,
                                                            'ep_aria': ep_aria, 'stl_large': True, 'host': host})
            else:
                logging.warning("totol is 0 " + response.url)
        # print(response, response.status, total_num)

    def parse_itemlinks(self, response, province, city, link_chengjiao, host, check):
        if check is True:
            itemdate = response.xpath("//div[@class = 'info']//div[@class = 'dealDate']/text()").getall()
            with open('check.csv', 'a+', encoding='utf-8-sig', newline="") as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([itemdate[len(itemdate)-1]])
    ...
    def parse_getitem(self, response, province, city, link_chengjiao, host):
        ...
        if not title:
            ...
            for r in range(len(record_price)):
                if ',' in record_detail[r]:
                    tem = record_detail[r].split(",")
                    record_list.append(record_price[r]+tem[0])
                    record_list.append(tem[1])
                else:
                    record_list.append(record_price[r])
                    record_list.append(record_detail[r])
